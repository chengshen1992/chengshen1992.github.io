...
author: "Mohamed Ismail and Shen Cheng"
date: last-modified
...
[Finch Studio](https://finchstudio.io/) is an innovative modeling workbench to perform pharmacometric analyses. Constructing together with [Mohamed Ismail](https://www.epd-llc.com/epd-team) from Enhanced Pharmacodynamics (ePD), this blog is illustating how to use Finch Studio on Minnesota Supercomputing Institute (MSI).
...
-   To visualize the results via Finch Studio, you would want to download (select and drag from right to left) everything from remote to local. Afterwards, the modeling results can be visualized using the Finch Studio toolkit (e.g., parameter estimations using `.xml` files; model diagnostics using `sdtab`, etc).
